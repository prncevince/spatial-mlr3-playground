...
```{r map-landslides-fig, echo=FALSE, cache=TRUE}
map
```
...
```{r map-landslides-glm-bias-pred-fig, echo=FALSE, cache=TRUE}
map_glm
```
...
```{r fig.screen=TRUE, cache=TRUE, cache=TRUE, dependson='train-auto'}
autoplot(
...
```{r preds_svm, cache=TRUE, cache=TRUE, dependson='train-auto'}
tic()
...
```{r maps-svm, cache=TRUE, dependson='preds_svm'}
...
```{r fig.screen=TRUE, cache=TRUE, dependson=c('maps-svm')}
tmap_arrange(map, map_glm, map_svm_spcv, map_svm_cv)
```

##### Prediction Data

```{r fig.screen=TRUE, cache=TRUE}
plot(ta)
```
